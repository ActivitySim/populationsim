...
class ConstructPipe:
    ...
    def construct_pipe(self):
        ...
        pipehouseholds = self.households.merge(self.xwalk[['MAZ', 'PUMA']], how = 'left', on = 'MAZ')
        ...
        pipe[r'/expanded_household_ids/expand_households'] = hh[['PUMA','TAZ','MAZ','hh_id']]
        pipe[r'/TAZ_weights/sub_balancing.geography=TAZ'] = pd.DataFrame(
            {
                x: [0] for x in list(self.xwalk.columns) + ['hh_id', 'balanced_weight', 'integer_weight']
                if x not in ['MAZ']
            }
        )
